...
much more favourable RMSE of \SI{0.55}{\log \micro M}.
...
expects the atomic distribution to converge much more rapidly than equilibrium
concentrations, if the micelle is pre-assembled, which means that the initial
configuration for the simulation constitutes a guess for the micelle's
structure: its shape and aggregation number.

TODO: Give metrics for this model, talk about the issues with molecular
conformations and the problem of pre-assembling micelles in this way. Discuss
COSMOplex metrics as well as the difficulty of local minima with respect to the
iterative procedure; the initial guess is very important.
