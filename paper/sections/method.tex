...
The basic topology of the neural network used in this work was identical to the
one used by \citet{qinPredictingCriticalMicelle2021}. That is, the first step of
the model consists of a stack of graph network layers, which mutate the node
features in a molecular graph based on those of bonded atoms. In this way, each
node feature vector encodes not only information about the atom itself, but its
neighbours. The next step is a pooling layer, which converts the graph to a
vector, losing the explicit topological information. We will call this the
latent representation vector; after training the model, it effectively acts as a
machine-learned representation of the molecule that captures only the
information about its topology and composition that is useful for predicting the
CMC. The final step is a readout neural network: a multi-layer perceptron which
acts as a nonlinear approximator to map this latent representation vector to the
CMC property prediction. The architecture is illustrated in Figure .

\begin{figure}
    \centering
    % \input{images/model-graph.tex}
\end{figure}
