...
we exploit the kernel function from the trained GP to create a
\emph{force-directed graph}. In this approach, each molecule is assigned a point
in 2D space. Each point is connected by a virtual `spring', whose equilibrium
length and spring constant depends on the value of the kernel function computed
between the molecules that the spring connects. The system is then
equilibriated, in a fashion similar to a molecular dynamics simulation. The resulting
layout is shown in Figure \ref{fig:fdg}.

\begin{figure}
    \includegraphics[width=\textwidth]{images/force-graph.pdf}
    \caption{A visualisation of the molecules in the combined NIST and Qin datasets using a force-directed graph layout. TODO: Explain this layout.}
    \label{fig:fdg}
\end{figure}
