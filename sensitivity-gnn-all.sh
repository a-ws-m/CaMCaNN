...
for i in "${!splits[@]}"; do
    ...
    base_dir_name="gnn-all-$num_splits-splits"
    ...
    for trial in $(seq 0 $((num_repeats * num_splits - 1))); do
        ...
        cp "models/gnn-search-all/best_hps.json" "$dir_name"
    done

    echo "Running python module"

    # Run the python module with the arguments
    python -m camcann.lab -e 500 --and-uq --only-best --splits "$num_splits" --repeats "$num_repeats" GNNModel All "$base_dir_name"
done
